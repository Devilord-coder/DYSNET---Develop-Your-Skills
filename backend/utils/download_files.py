...
def download_file(Auth_token, filename, disk_folder):
    ...
    download_response = requests.get(download_link, stream=True)  # stream=True — обязательно для больших файлов
    ...
    if download_response.status_code != 200:
        ...
        with open(download_path, 'wb') as f:
            for chunk in download_response.iter_content(chunk_size=8192):  # Скачиваем файл по кусочкам
                f.write(chunk)
