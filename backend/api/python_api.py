...
from backend.utils.check_task import check_task
...
def tasks():
    ...
    return render_template("python/tasks.html", tasks=tasks, level=session["level"])
    ...
    if request.method == "POST":
        ...
        for idx, test_data in enumerate(tests_data):
            ...
            if input_val or output_val:
                if input_val and output_val:
                    ...
                    test_errors.append(
                        f'Тест {idx + 1}: заполнены не все поля (ввод: {"есть" if input_val else "пусто"}, вывод: {"есть" if output_val else "пусто"})'
                    )
        ...
        for test_data in tests_data:
            ...
            if input_val and output_val:  # Сохраняем только полностью заполненные тесты
                test = PythonTest(task_id=task.id, args=input_val, result=output_val)
                db_sess.add(test)
                saved_tests_count += 1
...
def junior():
    ...
    return render_template("python/tasks.html", tasks=tasks, level="junior")
...
def middle():
    ...
    return render_template("python/tasks.html", tasks=tasks, level="middle")
...
def senior():
    ...
    return render_template("python/tasks.html", tasks=tasks, level="senior")
