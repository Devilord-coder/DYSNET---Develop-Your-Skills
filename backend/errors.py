...
def payment_required(e):
    return render_error_template(
        "fas fa-credit-card",
        "Требуется оплата",
        "Для доступа к этому ресурсу требуется оплата.",
        402,
    )
...
def proxy_auth_required(e):
    return render_error_template(
        ...
        "Необходима аутентификация для прокси-сервера.",
        407,
    )
...
def gone(e):
    return render_error_template(
        "fas fa-trash-alt", "Удалено", "Запрашиваемый ресурс больше не доступен.", 410
    )


def length_required(e):
    return render_error_template(
        "fas fa-ruler", "Требуется длина", "Заголовок Content-Length не определен.", 411
    )
...
def too_early(e):
    return render_error_template(
        "fas fa-clock",
        "Слишком рано",
        "Сервер не хочет рисковать обрабатывать запрос, который может быть повторен.",
        425,
    )
...
def variant_negotiates(e):
    return render_error_template(
        "fas fa-exchange-alt",
        "Вариант также согласовывает",
        "Выбранный вариант ресурса настроен на прозрачное согласование контента.",
        506,
    )
...
def not_extended(e):
    return render_error_template(
        "fas fa-expand",
        "Не расширено",
        "Для доступа к ресурсу требуются дополнительные расширения запроса.",
        510,
    )
